"""End-to-end test for the ``node-code-execution-distroless`` example.

Mirrors the manual steps from ``node-code-execution-distroless/README.md``:

1. ``unikraft build . --output <prefix>/node-code-exec-distroless:<tag>``
...
def _template_and_roms(request, unikraft, repo_root, ukc_image_prefix, test_run_id):
    ...
    """
    context = repo_root / "node-code-execution-distroless"
    base_tag = f"{ukc_image_prefix}/node-code-exec-distroless:{test_run_id}"
    template_name = f"{test_run_id}-tmpl"
