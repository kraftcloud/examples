"""End-to-end test for the ``nginx-flask-mongo-distroless`` example.

Mirrors the manual steps from ``nginx-flask-mongo-distroless/README.md``:
...
def test_nginx_flask_mongo(build_image, run_instance, http, unikraft, request, test_run_id, wait_instance):
    ...
    run_instance(
        ...
        name=f"mongo-distroless-{test_run_id}",
    )
    ...
    run_instance(
        ...
        name=f"flask-distroless-{test_run_id}",
    )
    ...
    nginx_instance = run_instance(
        ...
        memory="512M",
        name=f"nginx-distroless-{test_run_id}",
